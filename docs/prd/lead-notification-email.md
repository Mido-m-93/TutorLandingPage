...
- **Recipient**: a single fixed admin address (mohamada@roboco-op.org), not user-configurable through the UI. Must match the Resend account's own verified email exactly (including case) while using Resend's shared sandbox sender - a case mismatch causes Resend to reject the send.
